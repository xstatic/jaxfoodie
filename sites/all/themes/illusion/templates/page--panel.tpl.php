...
<div id="side_menu">
    <header class="m_bottom_30 d_table w_full">
        ...
        <div class="d_table_cell half_column v_align_m">
            <a href="index.html">
                <img src="<?php echo $theme_root; ?>/images/logo_side.png" alt="">
            </a>
        </div>
...
?> bg_light">
    ...
    <?php if ($title) : ?>
		...
		$blog_page = '';
		$portfolio_page = '';
		if(isset($array_curr_uri[2])) {
			$blog_page = substr($array_curr_uri[2], 0, 4);
			$portfolio_page = substr($array_curr_uri[2], 0, 9);
		}
        $class_name = 'page_title bg_gradient';
        if (count(array_intersect(array('blog'), $array_curr_uri)) > 0):
            $class_name = 'page_title translucent_bg_purple image_fixed image_bg_15';
        elseif (count(array_intersect(array('portfolio'), $array_curr_uri)) > 0):
            $class_name = 'page_title bg_gradient';
        else:
            //$class_title = strtolower(str_replace(' ','-',$title));	
            if(count(array_intersect(array('pages_about','pages_full_width','pages_right_sidebar'), $array_curr_uri)) > 0):
                    $class_name = 'page_title translucent_bg_color_scheme image_fixed image_bg_9';
            elseif (count(array_intersect(array('pages_left_sidebar'), $array_curr_uri)) > 0):
                    $class_name = 'page_title translucent_bg_color_dark image_fixed image_bg_9';
            elseif (count(array_intersect(array('pages_services'), $array_curr_uri)) > 0):
                    $class_name = 'page_title translucent_bg_purple image_fixed image_bg_10';
            elseif (count(array_intersect(array('pages_team'), $array_curr_uri)) > 0):
                    $class_name = 'page_title translucent_bg_orange image_fixed image_bg_11';
            elseif (count(array_intersect(array('pages_process'), $array_curr_uri)) > 0):
                    $class_name = 'page_title translucent_bg_color_dark image_fixed image_bg_12';
            elseif (count(array_intersect(array('pages_careers'), $array_curr_uri)) > 0):
                    $class_name = 'page_title translucent_bg_red image_fixed image_bg_13';
            elseif (count(array_intersect(array('pages_contact'), $array_curr_uri)) > 0):
                    $class_name = 'page_title translucent_bg_color_scheme image_fixed image_bg_14';
            elseif (count(array_intersect(array('pages_faq','pages_sitemap'), $array_curr_uri)) > 0):
                    $class_name = 'page_title translucent_bg_green';
            elseif (count(array_intersect(array('pages_404'), $array_curr_uri)) > 0):
                    $class_name = 'page_title translucent_bg_red image_fixed';
            elseif (count(array_intersect(array('shortcodes_elements'), $array_curr_uri)) > 0):
                    $class_name = 'page_title translucent_bg_purple';
            elseif (count(array_intersect(array('shortcodes_columns'), $array_curr_uri)) > 0):
                    $class_name = 'page_title translucent_bg_color_scheme';
            elseif (count(array_intersect(array('shortcodes_typography'), $array_curr_uri)) > 0):
                    $class_name = 'page_title translucent_bg_orange';
            elseif (count(array_intersect(array('shop_category_v1','shop_category_v2','product','cart','checkout','shop_wishlist','shop_compare_products','shop_order_list','shop_order_information','user'), $array_curr_uri)) > 0):
                    $class_name = 'page_title_2 bg_light_2';
            else:
                    $class_name = 'page_title bg_gradient';
            endif;
        endif;
        ?>
        <section class="<?php echo $class_name; ?> t_align_c relative wrapper">
            <?php if (count(array_intersect(array('pages_404'), $array_curr_uri)) > 0): ?>
                <img src="<?php echo $theme_root; ?>/images/page_404.jpg" alt="" class="page_404 d_xs_none">
            <?php else: ?>
                <div class="container">
                <?php endif; ?>
                <?php
                $page_title_2 = array('shop_category_v1', 'shop_category_v2', 'product', 'cart', 'checkout', 'shop_wishlist', 'shop_compare_products', 'shop_order_list', 'shop_order_information', 'user');
                if (count(array_intersect(array('shop_category_v1','shop_category_v2','product','cart','checkout','shop_wishlist','shop_compare_products','shop_order_list','shop_order_information','user'), $array_curr_uri)) > 0):
                    ?>
                    <h1 class="color_dark fw_light m_bottom_5"><?php print $title; ?></h1>
                <?php else: ?>
                    <h1 class="color_light fw_light m_bottom_5"><?php print $title; ?></h1>
                <?php endif; ?>
                <?php if (count(array_intersect(array('pages_404'), $array_curr_uri)) > 0): ?>
    ...
    <?php else: ?>
        <?php
        $other_pages = array('cart', 'checkout', 'contact', 'user', 'comments', 'comment', 'search', 'blog', 'product', 'product-category', 'product-tag', 'filter',
            'product-manufacturer', 'newsletter');
        ?>
...
<?php
if(isset($array_curr_uri[2])):
switch ($array_curr_uri[2]):
    case '':
    case 'home':
        echo 'jQuery("#art-revolution").addClass("r_slider");';
        break;
    default :
        break;
        ?>
